...
RD = Inet1.OpenURL("http://localhost/ao/u.php?a=list&hamachi=" & CInt(hamaa.Value) & "&v=" & App.Major & "." & App.Minor & "." & App.Revision)
...
MsgBox "Hay una nueva version disponible en la web, para poder jugar nesecitás descargarla."
    If MsgBox("¿Querés descargar la nueva version o parche ahora?", vbYesNo) = vbYes Then
        Call ShellExecute(0, "Open", "http://localhost/ao/?a=descargar&v=" & App.Major & "." & App.Minor & "." & App.Revision, "", App.path, 0)
    End If
End If
crearlista RD
End Sub
